...
function App() {
  const name = "Usama";
  const obj = {name: "Usama Anwar"};
  const data = ['We', 'are', 'United'];
  const list = [{name: "Usama"}, {name: "Anwar"}, {name: "Zahir"}];
  const complex = [
  ...
  return (
    <div>
      ...
      <p><b>Showing List Of Array:</b></p>
      <h2>{data[0]}</h2>
      <h2>{data[1]}</h2>
      <h2>{data[2]}</h2>
      <p><b>Showing List Of Array Object:</b></p>
      <p>This is {Object.values(list[0])}</p>
      <p>This is {Object.values(list[1])}</p>
      <p>This is {Object.values(list[2])}</p>
    <p>
      Usama is doing{complex[0].jobs} job in {complex[0].company}<br/><br/>
      Zahir is doing{complex[0].jobs} job in {complex[0].company}
      </p>
...
The default exports tend to be used for whatever you normally expect to get from the module. The named exports tend to be used for utilities that might be handy, but aren't always necessary. However it is up to you to choose how to export things: for example, a module might have no default export at all.`}
      ...
      {post.map((data) => {
          return (
            <div>
              <h1>{data.id} {data.title.toUpperCase()}</h1>
              <p>{data.body}</p>

              {user.map((user_Id) => {
                if (data.userId === user_Id.id) {
                  return (
                    <div>
                      <p alt={user_Id.username}>Posted by: <a href={"http://" + user_Id.website} target="_blank">{user_Id.name.toUpperCase()}</a></p>
                      <p>Contact Email: <a href={"mailto:" + user_Id.email}>{user_Id.email}</a></p>
                    </div>
                  )
                }
